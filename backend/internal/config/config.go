...
func LoadConfig() *Config {
	...
	config := &Config{
		...
		// Base de datos
		DBHost:     getEnv("DB_HOST", "sistema-tours-db2"),
...
type Config struct {
	...
	// Base de datos (Se conecta internamente al puerto 5432 en el contenedor)
	DBHost     string
	DBPort     string
	DBName     string
	DBUser     string
...
func LoadConfig() *Config {
	...
	config := &Config{
		...
		// Base de datos.
		// DB_HOST debe ser el nombre del servicio, en este caso "sistema-tours-db2".
		// DB_PORT se usa para la conexión interna (que seguirá siendo 5432, a pesar de un puerto host externo diferente).
